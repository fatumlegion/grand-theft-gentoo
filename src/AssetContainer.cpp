...
void AssetContainer::addBigTexture(std::string name, std::string path)
{
	cont_bigTextures[name] = new thor::BigTexture();
	cont_bigTextures[name]->loadFromFile(path.c_str());
}
...
thor::BigTexture *AssetContainer::getBigTexture(std::string name)
{
	if (!cont_bigTextures[name])
		printf("BigTexture %s doesn't exist.\n", name.c_str());
	return cont_bigTextures[name];
}
